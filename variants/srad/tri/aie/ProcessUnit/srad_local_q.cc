...
inline void compute_q2_vec(v8float j_c,
                           v8float j_n,
                           v8float j_s,
                           v8float j_w,
                           v8float j_e,
    ...
    q2_v = aie::select(q2_v, one_v, q_den_zero_mask);
    q2_v = aie::select(q2_v, one_v, center_zero_mask);
    auto valid_v = aie::select(one_v, zero_v, q_den_zero_mask);
    valid_v = aie::select(valid_v, zero_v, center_zero_mask);

    *q2_out = q2_v.to_native();
    *valid_out = valid_v.to_native();
}

} // namespace

extern "C" void srad_local_q(srad_local_q_input_buffer& in_j,
                             output_buffer<float>& out_q2) {
#if SRAD_AIE_DEBUG && (defined(__AIESIM__) || defined(__X86SIM__))
    ...
    float* __restrict mid = out_q2.data();
    float* __restrict center_q2 =
        mid + kCenterQ2Plane * srad_cfg::kMidPlaneStride;
    float* __restrict center_valid =
        mid + kCenterValidPlane * srad_cfg::kMidPlaneStride;
    float* __restrict south_q2 =
        mid + kSouthQ2Plane * srad_cfg::kMidPlaneStride;
    float* __restrict south_valid =
    ...
    for (int chunk = 0; chunk < kChunksPerRow; ++chunk)
        ...
        v8float center_q2_vec;
        v8float center_valid_vec;
        compute_q2_vec(center_c, center_n, center_s, center_w, center_e,
                       zero, one, neg_one, quarter, half, one_sixteenth,
                       zero_v, one_v,
                       &center_q2_vec, &center_valid_vec);

        v8float south_q2_vec;
        v8float south_valid_vec;
        compute_q2_vec(south_c, south_n, south_s, south_w, south_e,
                       zero, one, neg_one, quarter, half, one_sixteenth,
                       zero_v, one_v,
                       &south_q2_vec, &south_valid_vec);
        ...
        store_vec(south_valid, col,
                  select_data_lanes(south_valid_vec, data_mask, zero_v));
...
#if SRAD_AIE_DEBUG && (defined(__AIESIM__) || defined(__X86SIM__))
    if (call_id < 8) {
        std::printf("[srad_local_q] exit call=%d q2_0=%.9g\n",
                    call_id,
                    center_q2[0]);
    }
#endif
