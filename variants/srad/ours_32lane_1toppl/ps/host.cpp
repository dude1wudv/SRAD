...
constexpr const char* kBuildTag = "ours_32lane-single-batch-20260623";
...
int main(int argc, char* argv[]) {
    ...
    std::printf("[host] lanes        : %d column lanes, strip batches=%d\n",
                srad_cfg::kParallelLanes,
                srad_cfg::kBoardStripBatches);
    ...
    try {
        ...
        stage_t0 = Clock::now();
        log_stage("before toppl_run.wait");
        toppl_run.wait();
        log_stage("after toppl_run.wait");
        ...
        std::printf("timing us: h2d_us=%lld submit_us=%lld graph_init_us=%lld graph_run_submit_us=%lld toppl_launch_us=%lld pl_total_us=%lld toppl_wait_us=%lld graph_wait_us=%lld graph_end_us=%lld d2h_us=%lld total_us=%lld\n",
