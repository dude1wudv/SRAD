...
class SimSemanticsTest(unittest.TestCase):
    ...
    def test_graph_uses_single_input_plio_into_k1_only(self):
        graph = (
        ...
        self.assertIn("static constexpr int kInputCount = 1", graph)
        ...
        self.assertNotIn("core.in[1]", graph)
    ...
    def test_mid_buffer_uses_aligned_physical_row_stride(self):
        ...
        self.assertEqual(cfg["kCoeffPlanesPerRow"], 4)
        self.assertEqual(cfg["kRawRowsPerMidPacket"], 3)
        self.assertEqual(cfg["kRawRowStartPlane"], cfg["kCoeffPlanesPerRow"])
        self.assertEqual(cfg["kMidRecordElems"], 7)
        self.assertEqual(
            cfg["kMidElemsPerRow"],
        ...
        self.assertIn("kRawNorthPlane", local_q)
        self.assertIn("kRawCenterPlane", local_q)
        self.assertIn("kRawSouthPlane", local_q)
        self.assertIn("kRawNorthPlane", coeff_update)
        self.assertIn("kRawCenterPlane", coeff_update)
        self.assertIn("kRawSouthPlane", coeff_update)
    ...
    def test_mid_buffer_reuses_center_plane_for_east_coefficients(self):
        ...
        self.assertIn(
            "constexpr int kMidRecordElems = "
            "kCoeffPlanesPerRow + kRawRowsPerMidPacket;",
            (
            root / "aie" / "Config.h"
            ).read_text(encoding="utf-8"),
        )
        self.assertIn("constexpr int kCoeffPlanesPerRow = 4;", (
            root / "aie" / "Config.h"
        ).read_text(encoding="utf-8"))
        self.assertIn("constexpr int kRawRowsPerMidPacket = 3;", (
            root / "aie" / "Config.h"
        ).read_text(encoding="utf-8"))
        self.assertIn(
    ...
    def test_local_q_and_update_hoist_q0_and_split_hot_paths(self):
        ...
        self.assertIn("store_vec(raw_north, col, load_vec(row_m_minus1, col));", local_q)
        self.assertIn("store_vec(raw_center, col, load_vec(row_m, col));", local_q)
        self.assertIn("store_vec(raw_south, col, load_vec(row_m_plus1, col));", local_q)

        self.assertIn("const float* __restrict mid = in_c.data();", coeff_update)
        self.assertIn("const float* __restrict row_m_minus1 =", coeff_update)
        self.assertIn("const float* __restrict row_m =", coeff_update)
        self.assertIn("const float* __restrict row_m_plus1 =", coeff_update)
        ...
        self.assertNotIn("srad_update_input_buffer", coeff_update)
