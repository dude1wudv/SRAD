...
def parse_pkt_word32(token: str) -> int:
    ...
    value = float(token)
    if value.is_integer() and 0.0 <= value <= float(0xFFFFFFFF):
        int_value = int(value)
        if int_value >= 65536:
...
def read_pktstream_words(path: Path) -> list[int]:
    if not path.exists():
        raise FileNotFoundError(path)
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not fields:
            continue
        # TopPL receives each pktstream word in the low 32 bits of a 64-bit
        # PLIO word. Some simulator dumps include the unused high half.
        words.append(parse_pkt_word32(fields[0]))
    return words
...
def resolve_lane_output_paths(path: Path, lanes: int) -> list[Path]:
    if lanes <= 1:
        return [resolve_aie_output_path(path)]

    stem = path.stem
    match = re.match(r"^(.*)_\d+$", stem)
    prefix = match.group(1) if match else stem
    return [
        resolve_aie_output_path(path.with_name(f"{prefix}_{lane}{path.suffix}"))
        for lane in range(lanes)
    ]


def decode_packet_way(header: int, merge_ways: int) -> int:
    if merge_ways <= 0 or (merge_ways & (merge_ways - 1)) != 0:
        raise RuntimeError(f"merge ways must be a power of two, got {merge_ways}")
    return header & (merge_ways - 1)


def decode_merged_pktstream_words(words: list[int],
                                  cfg: dict[str, int]) -> list[list[float]]:
    merge_ways = cfg["kOutputMergeWays"]
    row_phys = cfg.get("kOutputRowPhysElems", cfg["kRowPhysElems"])
    packet_words = 1 + row_phys
    row_words = merge_ways * packet_words

    if len(words) % row_words != 0:
        raise RuntimeError(
            f"merged pktstream has {len(words)} 32-bit words, expected a "
            f"multiple of {row_words} ({merge_ways} packets x "
            f"{packet_words} words)"
        )

    rows = len(words) // row_words
    decoded: list[list[float]] = [[] for _ in range(merge_ways)]
    pos = 0
    for row in range(rows):
        seen: set[int] = set()
        for _packet in range(merge_ways):
            header = words[pos]
            pos += 1
            way = decode_packet_way(header, merge_ways)
            if way in seen:
                raise RuntimeError(
                    f"duplicate packet way {way} in merged row {row}; "
                    "check packet ID mapping against packet_ids_c.h"
                )
            seen.add(way)
            payload = words[pos:pos + row_phys]
            pos += row_phys
            decoded[way].extend(bits_to_float(word) for word in payload)

        if len(seen) != merge_ways:
            raise RuntimeError(
                f"merged row {row} produced {len(seen)} packet ways, "
                f"expected {merge_ways}"
            )
    return decoded
...
def compare_rowstream(got: list[float],
    ...
    print(f"{label} compared rows : {rows_per_iter - invalid_rows} per iter")
    print(f"{label} compared floats: {compared_count}")
    print(f"{label} max abs diff   : {max_abs:.9g}")
    if max_i >= 0:
        iter_span = rows_per_iter * row_phys
        iter_idx = max_i // iter_span
        row = (max_i % iter_span) // row_phys
        col = max_i % row_phys
        print(f"{label} max location : iter {iter_idx}, row {row}, col {col}")
        print(f"{label} output       : {got[max_i]:.9g}")
        print(f"{label} python gold  : {gold[max_i]:.9g}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--aie-output",
        type=Path,
        default=base / "aiesim_j_next_merged_0.txt",
    )
    ...
    ok = True
    merged_outputs = resolve_lane_output_paths(
        args.aie_output,
        cfg["kMergedOutputPlioCount"],
    )
    for merge, aie_output in enumerate(merged_outputs):
        merged_words = read_pktstream_words(aie_output)
        lane_streams = decode_merged_pktstream_words(merged_words, cfg)
        for way, got in enumerate(lane_streams):
            lane = merge * cfg["kOutputMergeWays"] + way
            lane_ok = compare_rowstream(
                got, gold_stream, cfg, args.iters, args.tol, f"lane {lane}"
            )
            print(f"lane {lane} output  : {aie_output} packet way {way}")
            ok = ok and lane_ok
