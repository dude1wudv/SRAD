...
class SimSemanticsTest(unittest.TestCase):
    ...
    def test_toppl_limits_m_axi_adapter_buffering_for_timing(self):
        ...
        for port, bundle in (
            ...
            self.assertRegex(
                ...
                r"max_read_burst_length=64 max_write_burst_length=64\s+"
                r"num_read_outstanding=2 num_write_outstanding=2",
            )
    ...
    def test_toppl_moves_m_axi_buffers_out_of_bram(self):
        toppl_cfg = (
            Path(__file__).resolve().parents[1] / "pl" / "TopPL.cfg"
        ).read_text(encoding="utf-8")

        self.assertIn("syn.interface.m_axi_buffer_impl=uram", toppl_cfg)
