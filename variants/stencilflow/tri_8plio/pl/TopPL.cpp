...
#define TRI8PLIO_VOLUME_DDR_WORDS 16384
...
constexpr int kWarmupRows = 4;
constexpr int kStencilRadiusRows = kWarmupRows / 2;
constexpr int kIterations = kRowsPerLane + kWarmupRows;
...
constexpr int kVolumeDdrWords = TRI8PLIO_VOLUME_DDR_WORDS;

static_assert(kCols == 256, "tri_8plio TopPL expects 256 columns");
static_assert(kNumLanes == 8, "tri_8plio TopPL expects 8 AIE lanes");
static_assert(kGridDepth * kLanesPerDepth == kNumLanes,
              "lane count must match depth split");
static_assert(kWarmupRows == 4, "tri_8plio TopPL expects four warmup rows");
...
#pragma HLS INTERFACE m_axi port=input offset=slave bundle=gmem0 depth=TRI8PLIO_VOLUME_DDR_WORDS
#pragma HLS INTERFACE m_axi port=output offset=slave bundle=gmem1 depth=TRI8PLIO_VOLUME_DDR_WORDS
