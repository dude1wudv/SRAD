...
void hdiff_lap(lap_input_buffer& in_window,
  ...
  const v8int32 zero_chunk = null_v8int32();

  for (unsigned i = COL / 8 - 2; i < COL / 8; i++)
    chess_prepare_for_pipelining
    chess_loop_range(1, ) {
      v16int32 flux_sub;
      v8int32 row0_next = zero_chunk;
      v8int32 row1_next = zero_chunk;
      v8int32 row2_next = zero_chunk;
      v8int32 row3_next = zero_chunk;
      v8int32 row4_next = zero_chunk;

      if (i + 1 < COL / 8) {
        row0_next = load_row_chunk(row0_base, i + 1);
        row1_next = load_row_chunk(row1_base, i + 1);
        row2_next = load_row_chunk(row2_base, i + 1);
        row3_next = load_row_chunk(row3_base, i + 1);
        row4_next = load_row_chunk(row4_base, i + 1);
      }
      ...
      data_buf2 = upd_w(data_buf2, 0, load_row_chunk(row2_base, i));
      data_buf2 = upd_w(data_buf2, 1, row2_next);
      ...
      data_buf1 = upd_w(data_buf1, 0, load_row_chunk(row1_base, i));
      data_buf1 = upd_w(data_buf1, 1, row1_next);
      ...
      data_buf2 = upd_w(data_buf2, 0, load_row_chunk(row0_base, i));
      data_buf2 = upd_w(data_buf2, 1, row0_next);
      ...
      data_buf2 = upd_w(data_buf2, 0, load_row_chunk(row4_base, i));
      data_buf2 = upd_w(data_buf2, 1, row4_next);
      ...
      data_buf1 = upd_w(data_buf1, 0, load_row_chunk(row3_base, i));
      data_buf1 = upd_w(data_buf1, 1, row3_next);

      acc_0 = lmsc8(acc_0, data_buf1, 2, 0x76543210, coeffs,      0, 0x00000000);
      acc_0 = lmac8(acc_0, data_buf1, 1, 0x76543210, coeffs_rest, 0, 0x00000000);

      data_buf2 = upd_w(data_buf2, 0, row1_next);
      data_buf2 = upd_w(data_buf2, 1, zero_chunk);
      ...
      data_buf1 = upd_w(data_buf1, 0, row3_next);
      data_buf1 = upd_w(data_buf1, 1, zero_chunk);
    }
}
