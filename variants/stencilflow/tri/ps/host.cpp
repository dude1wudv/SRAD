...
constexpr int ROWS_PER_GRAPH_RUN = hdiff_cfg::kBatchRows;
...
int main(int argc, char* argv[]) {
    ...
    const std::string output_path =
        (argc >= 5) ? argv[4] : "./aie_out_tri_256x256x64.txt";
    ...
    if (iter_cnt % ROWS_PER_GRAPH_RUN != 0) {
        std::fprintf(stderr,
                     "[error] iter_cnt %d must be divisible by kBatchRows %d\n",
                     iter_cnt,
                     ROWS_PER_GRAPH_RUN);
        return EXIT_FAILURE;
    }
    ...
    try {
        ...
        const int graph_iters = iter_cnt / ROWS_PER_GRAPH_RUN;
        topStencil.run(graph_iters);
        ...
        std::printf("pl_total_ms : %.3f\n", static_cast<double>(pl_us) / 1000.0);
