...
inline void zero_flux_forward_pack(output_buffer<int32_t>& out_buf) {
  int32_t* out = out_buf.data();
  for (unsigned i = 0; i < hdiff_cfg::kFluxForwardPackElems; ++i) {
    out[i] = 0;
  }
}
...
void hdiff_lap(
  ...
  int32_t* __restrict pack_base = flux_forward_pack.data();
  int32_t* __restrict out_flux1_base = pack_base + (0 * COL);
  int32_t* __restrict out_flux2_base = pack_base + (1 * COL);
  int32_t* __restrict out_flux3_base = pack_base + (2 * COL);
  int32_t* __restrict out_flux4_base = pack_base + (3 * COL);

  int32_t* __restrict base_ptr = in_window.data();
  int32_t* __restrict row0_base = base_ptr + (0 * COL);
  int32_t* __restrict row1_base = base_ptr + (1 * COL);
  int32_t* __restrict row2_base = base_ptr + (2 * COL);
  int32_t* __restrict row3_base = base_ptr + (3 * COL);
  int32_t* __restrict row4_base = base_ptr + (4 * COL);
