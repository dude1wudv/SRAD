...
def write_lane_map_txt(path: Path,
                       *,
                       lanes: int,
                       depth: int,
                       grid_rows: int) -> None:
    lanes_per_depth, output_rows_per_lane, lane_input_rows = lane_schedule(
        lanes=lanes,
        depth=depth,
        grid_rows=grid_rows,
    )
    ...
    with path.open("w", encoding="utf-8") as f:
        ...
        for lane in range(lanes):
            ...
            out_start = part * output_rows_per_lane
            out_end = out_start + output_rows_per_lane - 1
            in_start = out_start - STENCIL_RADIUS_ROWS
            in_end = in_start + lane_input_rows - 1
            f.write(
                f"{lane} {d} {part} "
                f"{in_start}..{in_end} "
                f"{out_start}..{out_end} "
                f"{WARMUP_ROWS}..{lane_input_rows - 1}\n"
            )
...
def lane_schedule(*,
                  lanes: int,
                  depth: int,
                  grid_rows: int) -> tuple[int, int, int]:
    if lanes % depth != 0:
        raise ValueError(f"lanes ({lanes}) must be a multiple of depth ({depth})")

    lanes_per_depth = lanes // depth
    if lanes_per_depth <= 0:
        raise ValueError("lanes_per_depth must be positive")
    ...
    output_rows_per_lane = grid_rows // lanes_per_depth
    lane_input_rows = output_rows_per_lane + WARMUP_ROWS
    if lane_input_rows < 5:
        raise ValueError("lane input stream must contain at least 5 rows")

    return lanes_per_depth, output_rows_per_lane, lane_input_rows


def make_lane_input_rows(plane: np.ndarray,
                         out_start: int,
                         output_rows_per_lane: int) -> np.ndarray:
    plane = np.asarray(plane, dtype=np.int32)
    if plane.ndim != 2 or plane.shape[1] != COL:
        raise ValueError(f"plane must be shape (N, {COL}), got {plane.shape}")

    grid_rows = plane.shape[0]
    lane_input_rows = output_rows_per_lane + WARMUP_ROWS
    src_start = out_start - STENCIL_RADIUS_ROWS
    rows = np.zeros((lane_input_rows, COL), dtype=np.int32)

    for i in range(lane_input_rows):
        src_row = src_start + i
...
def gold_from_depth_slices(slices3d: np.ndarray,
                           *,
                           lanes: int,
                           oob_mode: str = "zero") -> np.ndarray:
    slices3d = np.asarray(slices3d, dtype=np.int32)
    depth, grid_rows, cols = slices3d.shape
    if cols != COL:
        raise ValueError(f"slices3d must have cols={COL}, got {cols}")

    lanes_per_depth, output_rows_per_lane, _ = lane_schedule(
        lanes=lanes,
        depth=depth,
        grid_rows=grid_rows,
    )

    out = np.zeros((depth, grid_rows, COL), dtype=np.int32)
    for d in range(depth):
        for part in range(lanes_per_depth):
            out_start = part * output_rows_per_lane
            lane_rows = make_lane_input_rows(
                slices3d[d],
                out_start=out_start,
                output_rows_per_lane=output_rows_per_lane,
            )
            lane_out = hdiff_multi_windows_exact(lane_rows, oob_mode=oob_mode)
            out[d, out_start:out_start + output_rows_per_lane] = lane_out

    return out.reshape(depth * grid_rows, COL).astype(np.int32)


def dump_human_readable_inputs_depthwise(slices3d: np.ndarray, out_dir: Path, graph_id: str = "hdiff") -> None:
    depth, grid_rows, _ = slices3d.shape

    for d in range(depth):
        write_matrix_txt(out_dir / f"{graph_id}_plane{d}.txt", slices3d[d])


def dump_aie_stream_inputs_depthwise(slices3d: np.ndarray, out_dir: Path, graph_id: str = "hdiff") -> None:
    write_stream_txt(out_dir / f"{graph_id}_input_stream.txt",
                     np.asarray(slices3d, dtype=np.int32).reshape(-1))


def dump_lane_plio_inputs_depthwise(slices3d: np.ndarray,
                                    out_dir: Path,
                                    lanes: int,
                                    prefix: str = "input_plio") -> None:
    depth, grid_rows, _ = slices3d.shape
    lanes_per_depth, output_rows_per_lane, _ = lane_schedule(
        lanes=lanes,
        depth=depth,
    ...
    for lane in range(lanes):
        ...
        write_stream_txt(out_dir / f"{prefix}{lane}.txt", lane_rows.reshape(-1))
...
def main() -> None:
    ...
    ap.add_argument("--minimal", action="store_true",
                    help="generate only lane PLIO files and lane_map.txt")
    ap.add_argument("--emit-input-matrix", action="store_true",
                    help="also write input.txt as the full depth-major source matrix")
    ap.add_argument("--emit-debug-streams", action="store_true",
                    help="also write per-plane debug files and flattened debug stream")
    ap.add_argument("--emit-debug", action="store_true",
                    help="also write input.txt, per-plane debug files, and debug stream")
    ap.add_argument("--skip-lane-plio", action="store_true",
                    help="do not write per-lane input_plio*.txt files")
    ...
    ap.add_argument("--iter", type=int, default=None, dest="iter_cnt",
                    help="optional host iter count; default = rows_per_lane + warmup")
    ...
    ap.add_argument("--plio-prefix", default="input_plio")
    ap.add_argument("--oob-mode", choices=["zero", "clamp", "periodic"], default="zero")
    args = ap.parse_args()

    if args.minimal:
        args.skip_gold = True
    emit_input_matrix = args.emit_input_matrix or args.emit_debug
    emit_debug_streams = args.emit_debug_streams or args.emit_debug
    ...
    if emit_input_matrix:
        write_matrix_txt(data_dir / "input.txt", slices3d.reshape(-1, COL))
    ...
    if not args.skip_lane_plio:
        dump_lane_plio_inputs_depthwise(
            slices3d, data_dir, lanes=args.lanes, prefix=args.plio_prefix
        )
    write_lane_map_txt(
        data_dir / "lane_map.txt",
        lanes=args.lanes,
        depth=args.depth,
        grid_rows=args.grid_rows,
    )
    ...
    print(f"lanes/depth      : {lanes_per_depth}")
    print(f"rows/lane output : {output_rows_per_lane}")
    print(f"rows/lane input  : {lane_input_rows}")
    print(f"warmup rows      : {WARMUP_ROWS}")
    print(f"source rows/plane: {args.grid_rows}")
    if emit_input_matrix:
        print(f"input matrix     : {data_dir / 'input.txt'}")
    if emit_debug_streams:
        print(f"debug streams    : {args.graph_id}_plane*.txt, {args.graph_id}_input_stream.txt")
    if not args.skip_gold:
        print(f"gold output      : {gold.shape[0]} x {gold.shape[1]}")
    print(f"stream length    : each is {args.iter_cnt * COL} scalars")
    if args.skip_lane_plio:
        print("lane plio files  : skipped")
    else:
        print(f"lane plio files  : {args.plio_prefix}0.txt ... "
              f"{args.plio_prefix}{args.lanes - 1}.txt")
