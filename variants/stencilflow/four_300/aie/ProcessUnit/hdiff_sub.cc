...
void hdiff_sub(sub_lap_input_buffer& in_lap_package,
               output_buffer<int32_t>& out_sub_for_mscom,
               output_buffer<int32_t>& out_sub_for_selupdate) {
  ...
  int32_t* __restrict ms_base  = out_sub_for_mscom.data();
  int32_t* __restrict sel_base = out_sub_for_selupdate.data();
  ...
  if (use_prev_lap_mid) {
    ...
    for (unsigned i = 0; i < COL / 8; ++i)
      ...
      chess_loop_range(1, ) {
        ...
        v8int32 left_v  = sub_vec(mid_v,      lap_left[i]);
        v8int32 right_v = sub_vec(lap_right[i], mid_v);
        v8int32 up_v    = sub_vec(mid_v,      mid_v);
        v8int32 down_v  = sub_vec(lap_down[i], mid_v);
