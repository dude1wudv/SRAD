...
void hdiff_lap(lap_complete_input_buffer& in_window,
  ...
  // Reuse-oriented complete path.
  //
  // Lap emits the current row values sub needs plus lap_mid as the final row:
  //   [lap_left(row r) | lap_right(row r) | lap_down(row r) | lap_mid(row r)]
  //
  // The final lap_mid row is reused by hdiff_sub through a one-row margin on
  // the next firing, where it becomes lap_up for the new current row.
