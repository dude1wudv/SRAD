...
class SimSemanticsTest(unittest.TestCase):
    def test_make_sim_uses_six_rows_plus_flush_per_iteration(self):
        ...
        self.assertEqual(expected_plio64_lines, 1024)
    ...
    def test_toppl_decouples_ddr_and_aie_streams_with_local_fifos(self):
        ...
        self.assertIn("constexpr int kLocalStreamDepth = 2 * kWordsPerRow;", toppl)
        self.assertIn("static_assert(kLocalStreamDepth == 256", toppl)
        self.assertIn(
            "#pragma HLS STREAM variable=to_aie_words depth=256",
            toppl,
        )
        self.assertIn(
            "#pragma HLS STREAM variable=from_aie_words depth=256",
            toppl,
        )
        self.assertRegex(
            toppl,
    ...
    def test_board_config_uses_250_data_columns_padded_to_256(self):
        cfg, _lambda_default, _bypass = verify_srad.read_config()

        self.assertEqual(cfg["kRows"], 250)
        self.assertEqual(cfg["kCols"], 250)
        self.assertEqual(cfg["kRowDataElems"], 250)
        self.assertEqual(cfg["kRowPhysElems"], 256)
        self.assertEqual(cfg["kQ0PadIndex"], 250)
        self.assertEqual(cfg["kStatSumPadIndex"], 250)
        self.assertEqual(cfg["kStatSum2PadIndex"], 251)
        self.assertEqual(cfg["kBoardRows"], 4000)
        self.assertEqual(cfg["kBoardCols"], 4000)
        self.assertEqual(cfg["kBoardIterations"], 100)
        self.assertEqual(cfg["kBoardStrips"], 16)
