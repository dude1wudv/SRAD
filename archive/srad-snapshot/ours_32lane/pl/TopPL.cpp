...
static_assert(srad_cfg::kTopPlWorkers == 2,
              "ours_32lane board path expects two TopPL CUs");
...
static_assert(srad_cfg::kBoardStripBatches == 1,
              "32 AIE lanes should cover all board strips in one graph batch");
...
#pragma HLS INLINE
    return clamp_worker_id(worker_id) * srad_cfg::kWorkerCols;
}
...
#pragma HLS INLINE off
    ...
    for (int r = 0; r < srad_cfg::kBoardRows; ++r) {
        const int row_base = r * srad_cfg::kBoardCols + col_base;
