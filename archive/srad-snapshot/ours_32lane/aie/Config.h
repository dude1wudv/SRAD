...
// 32 row-stream lanes. Each lane uses two AIE cores: local_q then coeff_update.
// Two TopPL CUs each drive 16 lanes over one contiguous 2000-column half image.
// Lanes are placed on AIE columns 1..8, four lanes per column, leaving column 0
// unused.
constexpr int kTopPlWorkers = 2;
...
static_assert((kBoardStrips % kTopPlWorkers) == 0,
              "board strips must divide evenly across TopPL workers");
constexpr int kBoardStripsPerTopPl = kBoardStrips / kTopPlWorkers;
...
static_assert(kWorkerCols * kTopPlWorkers == kBoardCols,
              "TopPL worker column ranges must exactly cover the image width");
static_assert(kBoardStrips == kParallelLanes,
              "32 AIE lanes should cover all 125-column board strips at once");
...
static_assert(kParallelLanes == 32,
              "ours_32lane expects exactly 32 AIE row-stream lanes");
static_assert(kTopPlWorkers == 2,
              "ours_32lane expects two TopPL workers");
static_assert(kLanesPerTopPl == 16,
              "each TopPL worker should drive 16 AIE row-stream lanes");
static_assert(kTotalAieCores == 64,
              "ours_32lane expects exactly 64 AIE kernels");
static_assert(kAieLaneCols * kParallelLanesPerCol == kParallelLanes,
              "32 lanes should occupy eight AIE columns");
