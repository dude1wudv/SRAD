...
def read_config_dims():
    ...
    return (
        ...
        values["kTopPlWorkers"],
        values["kLanesPerTopPl"],
        values["kWorkerCols"],
        values["kBoardRows"],
        values["kBoardCols"],
        lambda_default,
...
def main():
    ...
    for _ in range(SIM_ITERATIONS):
        ...
        stream.extend(make_rowstream_input(cur, q0sqr, ROWS_PER_ITER_SIM, SIM_ROWS))
        cur = srad_next(cur, q0sqr, SIM_ROWS)

    for lane in range(PARALLEL_LANES):
        write_plio64_stream(base / f"plio_ours_j_{lane}.txt", stream)
    ...
    print(
        f"toppl_workers={TOPPL_WORKERS}, lanes/toppl={LANES_PER_TOPPL}, "
        f"worker_cols={WORKER_COLS}"
    )
    print(f"PLIO lines per lane: input={len(stream) // 2}")
